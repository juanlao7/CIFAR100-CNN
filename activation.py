...
for i in activations:
    ...
    # Defining the model.
    model = Sequential()
    model.add(Conv2D(27, (3, 3), activation=activations[i], input_shape=input_shape))
    model.add(MaxPooling2D(pool_size=(2, 2)))
    model.add(Conv2D(81, (3, 3), activation=activations[i]))
    model.add(MaxPooling2D(pool_size=(2, 2)))
    model.add(Conv2D(135, (3, 3), activation=activations[i]))
    model.add(MaxPooling2D(pool_size=(2, 2)))
    model.add(Flatten())
    model.add(Dense(128, activation=activations[i]))
    model.add(Dense(128, activation=activations[i]))
